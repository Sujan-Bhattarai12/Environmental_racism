...
#                              filter(PNPL > quantile(los_angeles$PNPL, 0.8, na.rm = TRUE) & PM25 > quantile(los_angeles$PM25, 0.8, na.rm = TRUE))
# 
...
bird_crs_transformed <- st_transform(bird, crs = st_crs(LA_redlining))
...
result <- bird_redlined %>% group_by(holc_grade) %>% 
          mutate(percent_of_birds = as.character(round(n()/nrow(bird_redlined)*100), 2))

#noerror untill this point
library(tmap)
########aggregate 
tm_shape(LA_redlining)+
    tm_fill(fill = "holc_grade", palette = c("green", "blue", "yellow", 'black'), alpha = 1)+
    tm_shape(result)+
    tm_dots(col = 'percent_of_birds', palette=c('16' ='black', '19'='yellow', '29'='blue', '36' ='red')) + 
...
(5 points)
Based on the graph, Most A graded areas(green areas in the graph) has black dots(percentage of birds diversity within all redlined areas).
This suggest that the areas with low grade were high in bird diversity. The potential implication of this finding is that low graded areas offer unique habitat to bird species, thus policy makers should be attentive on this matter. Moreover, this challenges the belief that low graded areas have low diversity, thus more research is required so that biodiversity conservation plans can be included to Environmental plans.
